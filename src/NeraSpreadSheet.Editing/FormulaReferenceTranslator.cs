...
{
    public static string Translate(string formula, CellAddress sourceCell, CellAddress targetCell)
    {
        ...
        for (var index = 0; index < formula.Length;)
        {
            ...
            if (!inString && IsReferenceBoundaryBefore(formula, index) && TryReadReference(formula, index, out var consumed, out var reference))
            {
                builder.Append(TranslateReference(reference, rowDelta, columnDelta));
                index += consumed;
                continue;
            }
    ...
    private static bool TryReadReference(string text, int start, out int consumed, out ParsedReference reference)
    {
        consumed = 0;
        reference = default;
        var index = start;
        ...
        if (index == rowStart || !IsReferenceBoundaryAfter(text, index))
        {
            return false;
        }
    ...
    private static string TranslateReference(ParsedReference reference, int rowDelta, int columnDelta)
    {
        var row = reference.Address.RowIndex + (reference.AbsoluteRow ? 0 : rowDelta);
        var column = reference.Address.ColumnIndex + (reference.AbsoluteColumn ? 0 : columnDelta);
        if (row < 0 || row >= SpreadsheetLimits.MaxRows || column < 0 || column >= SpreadsheetLimits.MaxColumns)
        {
            return "#REF!";
        }
        ...
        var split = 0;
        while (split < a1.Length && char.IsAsciiLetter(a1[split]))
        {
            split++;
        }
    ...
    private static bool IsReferenceBoundaryBefore(string text, int index)
    {
        if (index == 0)
        {
            return true;
    ...
    private readonly record struct ParsedReference(CellAddress Address, bool AbsoluteRow, bool AbsoluteColumn);
}
