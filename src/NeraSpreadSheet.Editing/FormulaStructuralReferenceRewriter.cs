...
public static class FormulaStructuralReferenceRewriter
{
        ...
        out ParsedReference secondMapped)
    {
        ...
        var mappedFirstIndex = ascending ? mappedStart : mappedEnd;
        ...
        out ParsedReferenceExpression expression)
    {
        ...
        TryReadSheetQualifier(text, ref index, out var firstQualifier);
        if (!TryReadReference(text, ref index, out var firstReference))
        {
            return false;
        }
        ...
        {
            index++;
            TryReadSheetQualifier(text, ref index, out secondQualifier);
            if (!TryReadReference(text, ref index, out var parsedSecond))
            {
                return false;
            }
    ...
    private static string FormatReference(ParsedReference reference)
    {
