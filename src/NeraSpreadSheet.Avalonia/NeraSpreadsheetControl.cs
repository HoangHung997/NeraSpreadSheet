...
public sealed partial class NeraSpreadsheetControl : Panel, IDisposable
{
    public static readonly DirectProperty<NeraSpreadsheetControl, SpreadsheetSession?> SessionProperty =
        AvaloniaProperty.RegisterDirect<NeraSpreadsheetControl, SpreadsheetSession?>(
            nameof(Session), control => control.Session, (control, value) => control.Session = value);
    ...
    {
        IsVisible = false, AcceptsReturn = true,
        BorderThickness = new Thickness(1), Padding = new Thickness(2, 0),
    };
    ...
    /// <summary>Visual scale, independent of worksheet dimensions. Valid range: 25% to 400%.</summary>
    public double Zoom
    {
        get => _zoom;
        set
        {
            VerifyUsable();
            if (!double.IsFinite(value) || value < 0.25 || value > 4)
                throw new ArgumentOutOfRangeException(nameof(value), "Zoom must be between 0.25 and 4.");
    ...
    private SpreadsheetChromeMetrics Chrome =>
        SpreadsheetChromeGeometry.Calculate(DocumentWidth, DocumentHeight, _renderTheme);
    ...
    public override void Render(DrawingContext context)
    {
        ...
        var chrome = Chrome;
        if (chrome.BodyWidth <= 0 || chrome.BodyHeight <= 0) return;
        var snapshot = _scroll.Snapshot;
        var frame = _viewport.Compose(snapshot.OffsetX, snapshot.OffsetY,
            chrome.BodyWidth, chrome.BodyHeight, 128d, _renderTheme);
        _layout = frame.Layout;
        var displayList = SpreadsheetChromeDisplayListComposer.Compose(
            frame.DisplayList, frame.Layout, _session.Selection.Capture(), _renderTheme);
        using (context.PushClip(new Rect(Bounds.Size)))
        using (context.PushTransform(Matrix.CreateScale(_zoom, _zoom)))
            _renderer.Render(context, displayList);
        RenderedFrameCount++;
    }
    ...
    protected override Size MeasureOverride(Size availableSize)
    {
        ...
        return new Size(double.IsFinite(availableSize.Width) ? availableSize.Width : 640,
            double.IsFinite(availableSize.Height) ? availableSize.Height : 400);
    }

    protected override Size ArrangeOverride(Size finalSize)
    {
        _editor.Arrange(_editor.IsVisible ? _editorBounds : default);
        return finalSize;
    }
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        if (change.Property == BoundsProperty && !_disposed) Refresh();
    }
    ...
    protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e)
    {
        if (!_disposed)
        {
            ...
            var snapshot = _scroll.Snapshot;
            _scroll.Reset();
            _scroll.ScrollTo(snapshot.OffsetX, snapshot.OffsetY, false);
            DetachSession();
            _renderer.ClearCaches();
        }
        _attached = false;
    ...
    /// <summary>Queues document-pixel deltas; fractional offsets are preserved and coalesced per frame.</summary>
    public void QueuePrecisionScroll(double deltaX, double deltaY)
    {
        VerifyUsable();
        if (!double.IsFinite(deltaX) || !double.IsFinite(deltaY))
            throw new ArgumentOutOfRangeException(nameof(deltaX), "Scroll deltas must be finite.");
        if (_session is null) return;
        _scroll.QueueDelta(new ScrollDelta(deltaX, deltaY, ScrollInputKind.Precision));
        if (_attached && !_frameTimer.IsEnabled)
        {
            _frameClock.Restart();
            _frameTimer.Start();
        }
    ...
    public void ScrollTo(double offsetX, double offsetY)
    {
        ...
        UpdateExtent();
        _scroll.ScrollTo(Math.Clamp(offsetX, 0, Math.Max(0, ContentWidth - ViewportBodyWidth)),
            Math.Clamp(offsetY, 0, Math.Max(0, ContentHeight - ViewportBodyHeight)), false);
        UpdateEditorBounds();
        InvalidateVisual();
    ...
    {
        VerifyUsable();
        if (_session is null || _viewport is null) return false;
        UpdateExtent();
        ...
        var x = scroll.OffsetX;
        var y = scroll.OffsetY;
        if (address.ColumnIndex >= _session.View.FrozenColumns)
        {
            var left = Math.Min(frozen.Width, ViewportBodyWidth);
            if (cell.Left < left) x -= left - cell.Left;
            else if (cell.Right > ViewportBodyWidth) x += cell.Right - ViewportBodyWidth;
        }
        if (address.RowIndex >= _session.View.FrozenRows)
        {
            ...
            else if (cell.Bottom > ViewportBodyHeight) y += cell.Bottom - ViewportBodyHeight;
        }
        if (x == scroll.OffsetX && y == scroll.OffsetY) return false;
        ScrollTo(x, y);
        return true;
    }
    ...
    internal void AdvanceFrame(TimeSpan elapsed)
    {
        VerifyUsable();
        UpdateExtent();
        var result = _scroll.AdvanceFrame(elapsed, new ScrollBounds(
            Math.Max(0, ContentWidth - ViewportBodyWidth), Math.Max(0, ContentHeight - ViewportBodyHeight)));
        if (result.Changed)
        {
            UpdateEditorBounds();
            InvalidateVisual();
    ...
    private void AttachSession()
    {
        ...
        _session.ActiveWorksheetChanged += OnWorksheetChanged;
        _session.Selection.Changed += OnSelectionChanged;
        _session.View.Changed += OnViewChanged;
        _session.Editor.StateChanged += OnCanonicalEditorChanged;
        _session.Analytics.Changed += OnVisualChanged;
        _session.AnalyticsPlacements.Changed += OnVisualChanged;
    ...
    {
        DetachWorksheet();
        _worksheet = _session?.ActiveWorksheet;
        if (_worksheet is null) return;
        _worksheet.CellsChanged += OnVisualChanged;
        _worksheet.Dimensions.Changed += OnDimensionsChanged;
    }
    ...
    private void OnVisualChanged(object? sender, EventArgs e) => Refresh();

    private void UpdateExtent()
    {
        if (_session is null || _viewport is null) { ContentWidth = ContentHeight = 0; return; }
        var chrome = Chrome;
        var extent = _adaptive
            ? _viewport.GetAdaptiveNavigationExtent(_session.Selection.ActiveCell,
                new SizeD(chrome.BodyWidth, chrome.BodyHeight),
                new PointD(_scroll.Snapshot.OffsetX, _scroll.Snapshot.OffsetY),
...
{
    public SpreadsheetInteractionFailedEventArgs(Exception exception)
    {
