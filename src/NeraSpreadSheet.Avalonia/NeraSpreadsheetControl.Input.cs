...
public sealed partial class NeraSpreadsheetControl
{
    ...
    protected override void OnPointerWheelChanged(PointerWheelEventArgs e)
    {
        ...
        {
            Zoom = Math.Clamp(_zoom + Math.Sign(e.Delta.Y) * 0.1, 0.25, 4);
            return;
        }
        var dx = -e.Delta.X * 48 / _zoom;
        var dy = -e.Delta.Y * 48 / _zoom;
    ...
    protected override void OnPointerPressed(PointerPressedEventArgs e)
    {
        ...
        if (e.Handled || _disposed || _session is null || _viewport is null ||
            !e.GetCurrentPoint(this).Properties.IsLeftButtonPressed) return;
        var p = e.GetPosition(this);
        // Preserve native TextBox caret/selection handling.
        ...
        RunInput(() =>
        {
            ...
            var x = p.X / _zoom;
            var y = p.Y / _zoom;
            if (_layout is not null && SpreadsheetHeaderResizeGeometry.TryHitResizeHandle(
                x, y, DocumentWidth, DocumentHeight, _renderTheme, _layout, out var resize))
            {
                _resize = resize;
            ...
            var additive = PrimaryModifier(e.KeyModifiers);
            ...
            switch (hit.Region)
            {
                ...
                case SpreadsheetChromeRegion.RowHeader:
                    ...
                    {
                        if (shift) _session.Selection.ExtendRowsTo(row);
                        else _session.Selection.SelectRow(row, additive);
                    }
                    break;
                case SpreadsheetChromeRegion.ColumnHeader:
                    if (_viewport.TryHitTestColumn(hit.BodyX, snapshot.OffsetX, out var column))
                    {
                        if (shift) _session.Selection.ExtendColumnsTo(column);
                        else _session.Selection.SelectColumn(column, additive);
                    }
                    break;
                case SpreadsheetChromeRegion.Body:
                    ...
                    if (e.ClickCount >= 2) BeginEdit();
                    else { _draggingSelection = true; CapturePointer(e.Pointer); }
                    break;
            }
        });
    }
    ...
    protected override void OnPointerMoved(PointerEventArgs e)
    {
        ...
        else if (_draggingSelection)
        {
            var chrome = Chrome;
            var snapshot = _scroll.Snapshot;
            if (_viewport.TryHitTest(p.X / _zoom - chrome.RowHeaderWidth,
                p.Y / _zoom - chrome.ColumnHeaderHeight, snapshot.OffsetX, snapshot.OffsetY, out var address))
    ...
    protected override void OnKeyDown(KeyEventArgs e)
    {
        ...
        if (PrimaryModifier(e.KeyModifiers))
        {
            Action? command = e.Key switch
            {
                Key.Z => () => { _session.Undo(); },
                Key.Y => () => { _session.Redo(); },
                Key.C => () => { _session.Clipboard.CopyPrimarySelection(); },
                Key.X => () => { _session.Clipboard.CutPrimarySelection(); },
                Key.V => () => { _session.Clipboard.PasteAtActiveCell(); },
                Key.B => () => { _session.Styles.ToggleBold(); },
                Key.I => () => { _session.Styles.ToggleItalic(); },
                _ => null,
            };
        ...
        {
            Key.Left => (Row: 0, Column: -1), Key.Right => (Row: 0, Column: 1),
            Key.Up => (Row: -1, Column: 0), Key.Down => (Row: 1, Column: 0), _ => (Row: 0, Column: 0),
        };
        if (delta == default) return;
        e.Handled = true;
        MoveActiveCell(delta.Row, delta.Column, (e.KeyModifiers & KeyModifiers.Shift) != 0);
    }
    ...
    protected override void OnTextInput(TextInputEventArgs e)
    {
        ...
        if (e.Handled || _disposed || _session is null || IsEditing ||
            string.IsNullOrEmpty(e.Text) || e.Text.Any(char.IsControl)) return;
        e.Handled = true;
        RunInput(() => BeginEdit(e.Text));
    }

    private static bool PrimaryModifier(KeyModifiers modifiers) =>
    ...
    private void MoveActiveCell(int rowDelta, int columnDelta, bool extend)
    {
        ...
        var next = SpreadsheetVisibleCellNavigation.GetNextVisibleCell(
            _session.ActiveWorksheet, _session.Selection.ActiveCell, rowDelta, columnDelta);
        if (extend) _session.Selection.ExtendTo(next);
        else _session.Selection.SetActiveCell(next);
