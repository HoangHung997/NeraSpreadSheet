...
internal sealed class FormulaTokenizer
{
    ...
    private FormulaToken ReadNumber()
    {
            ...
            !double.IsFinite(value))
        {
            throw new FormatException(
                $"'{text}' is not a valid finite number.");
        }
    ...
    private FormulaToken ReadString()
    {
        ...
        throw new FormatException(
            "Formula string literal is not terminated.");
    }

    private FormulaToken ReadQuotedIdentifier()
    {
        _position++;
        var builder = new System.Text.StringBuilder();
        while (_position < _text.Length)
        ...
        throw new FormatException(
            "Quoted worksheet identifier is not terminated.");
    }
    ...
    private FormulaToken ReadError()
    {
        var start = _position++;
        while (_position < _text.Length &&
               IsErrorCharacter(_text[_position]))
        {
            _position++;
        }
        ...
        {
            throw new FormatException(
                $"'{text}' is not a valid formula error literal.");
        }

        return new FormulaToken(
            FormulaTokenKind.Error,
