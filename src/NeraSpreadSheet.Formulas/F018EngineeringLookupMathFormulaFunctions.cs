...
internal static class F018EngineeringLookupMathFormulaFunctions
{
    ...
    public static IEnumerable<IFormulaFunction> Create()
    {
        ...
        yield return Def("HLOOKUP",3,4,i=>EvalLookup(i,true),propagateArgumentErrors:false);yield return Def("VLOOKUP",3,4,i=>EvalLookup(i,false),propagateArgumentErrors:false);
    ...
    private static FormulaFunctionDefinition Def(string name,int min,int max,Func<FormulaFunctionInvocation,FormulaEvaluationResult> eval,FormulaFunctionVolatility vol=FormulaFunctionVolatility.Deterministic,bool propagateArgumentErrors=true)=>new(new FormulaFunctionDescriptor(new FormulaFunctionIdentity("NERA.BUILTIN",name),new FormulaFunctionVersion(1,0,0),FormulaFunctionApiVersion.Current,min,max,ScalarRange|FormulaFunctionCapabilities.ReturnsScalar,vol,propagateArgumentErrors:propagateArgumentErrors,argumentCountPolicy:FormulaFunctionArgumentCountPolicy.LogicalArguments),eval);
    ...
    private static FormulaEvaluationResult EvalLookup(FormulaFunctionInvocation i,bool horizontal)
    {
        if(!ScalarVal(i.Arguments[0],out var key))return Val();
        if(key.Kind==CellValueKind.Error)return FromValue(key);
        if(!Shape(i.Arguments[1],out var rows,out var cols,out var vals))return Val();
        if(!IntOrError(i.Arguments[2],out var idx,out var indexError))return indexError;
        var limit=horizontal?rows:cols;
        if(idx<1||idx>limit)return Ref();
        bool approx=true;
        if(i.Arguments.Count==4&&!BoolOrError(i.Arguments[3],out approx,out var booleanError))return booleanError;
        int count=horizontal?cols:rows,found=-1;
        for(int p=0;p<count;p++)
        {
            var v=vals[horizontal?p:p*cols];
            if(v.Kind==CellValueKind.Error)return FromValue(v);
            var c=Cmp(v,key);
            if(c==0){found=p;break;}
            if(approx&&c<=0)found=p;
        }
        if(found<0)return NA();
        return FromValue(vals[horizontal?(idx-1)*cols+found:found*cols+(idx-1)]);
    }
    ...
    private static bool IntOrError(FormulaFunctionArgument a,out int v,out FormulaEvaluationResult error){if(a.Kind==FormulaFunctionArgumentKind.Scalar&&a.ScalarValue.Kind==CellValueKind.Error){v=default;error=FromValue(a.ScalarValue);return false;}if(Int(a,out v)){error=null!;return true;}error=Val();return false;}
    ...
    private static bool BoolOrError(FormulaFunctionArgument a,out bool v,out FormulaEvaluationResult error){if(a.Kind==FormulaFunctionArgumentKind.Scalar&&a.ScalarValue.Kind==CellValueKind.Error){v=default;error=FromValue(a.ScalarValue);return false;}if(Bool(a,out v)){error=null!;return true;}error=Val();return false;}
    ...
    private static FormulaEvaluationResult FromValue(CellValue value)=>value.Kind==CellValueKind.Error?new FormulaEvaluationResult(value,FormulaErrorMapping.ToErrorCode(value),Array.Empty<FormulaDependency>()):FormulaEvaluationResult.Success(value);
