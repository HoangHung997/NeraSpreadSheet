...
public sealed class NeraFormulaEngine : IFormulaEngine
{
    ...
    public NeraFormulaEngine(IFormulaFunctionRegistry? functions = null) { _functions = functions ?? new BuiltInFormulaFunctionRegistry(); }
    ...
    public FormulaEvaluationResult Evaluate(string formula, IFormulaEvaluationContext context)
    {
        ...
        try
        {
            ...
            return error == FormulaErrorCode.None ? FormulaEvaluationResult.Success(value, dependencies) : new FormulaEvaluationResult(value, error, dependencies);
        }
        catch (FormatException) { return FormulaEvaluationResult.Failure(FormulaErrorCode.InvalidValue); }
        catch (OverflowException) { return FormulaEvaluationResult.Failure(FormulaErrorCode.InvalidValue); }
    }

    private CellValue EvaluateNode(FormulaNode node, IFormulaEvaluationContext context, List<FormulaDependency> dependencies) => node switch
    {
    ...
    private static CellValue EvaluateCell(CellNode cell, IFormulaEvaluationContext context, List<FormulaDependency> dependencies)
    {
    ...
    private CellValue EvaluateUnary(UnaryNode unary, IFormulaEvaluationContext context, List<FormulaDependency> dependencies)
    {
        ...
        if (!TryNumber(value, out var number)) return CellValue.FromError("#VALUE!");
    ...
    private CellValue EvaluateBinary(BinaryNode binary, IFormulaEvaluationContext context, List<FormulaDependency> dependencies)
    {
        ...
        if (binary.Operator == FormulaTokenKind.Concat) return CellValue.FromText(left.ToString() + right.ToString());
        ...
        if (!TryNumber(left, out var leftNumber) || !TryNumber(right, out var rightNumber)) return CellValue.FromError("#VALUE!");
        if (binary.Operator == FormulaTokenKind.Divide && Math.Abs(rightNumber) <= double.Epsilon) return CellValue.FromError("#DIV/0!");
    ...
    {
        if (string.Equals(function.Name, "IF", StringComparison.OrdinalIgnoreCase)) return EvaluateIf(function, context, dependencies);
        if (!_functions.TryResolve(function.Name, out var formulaFunction)) return CellValue.FromError("#NAME?");
        ...
        foreach (var argument in function.Arguments)
        {
            ...
            else values.Add(EvaluateNode(argument, context, dependencies));
        }
    ...
    private CellValue EvaluateIf(FunctionNode function, IFormulaEvaluationContext context, List<FormulaDependency> dependencies)
    {
        ...
        if (!TryBoolean(condition, out var isTrue)) return CellValue.FromError("#VALUE!");
        if (isTrue) return EvaluateNode(function.Arguments[1], context, dependencies);
        return function.Arguments.Count == 3 ? EvaluateNode(function.Arguments[2], context, dependencies) : CellValue.FromBoolean(false);
    }
    ...
    private static bool TryNumber(CellValue value, out double number)
    {
        switch (value.Kind)
        {
            case CellValueKind.Number: number = (double)value.RawValue!; return true;
            case CellValueKind.Boolean: number = (bool)value.RawValue! ? 1d : 0d; return true;
            case CellValueKind.Blank: number = 0d; return true;
    ...
    private static bool TryBoolean(CellValue value, out bool result)
    {
        switch (value.Kind)
        {
            case CellValueKind.Boolean: result = (bool)value.RawValue!; return true;
            case CellValueKind.Number: result = Math.Abs((double)value.RawValue!) > double.Epsilon; return true;
            case CellValueKind.Blank: result = false; return true;
    ...
    {
        if (TryNumber(left, out var leftNumber) && TryNumber(right, out var rightNumber)) return leftNumber.CompareTo(rightNumber);
    ...
    {
        if (value.Kind != CellValueKind.Error) return FormulaErrorCode.None;
