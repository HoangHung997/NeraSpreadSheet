...
private const int MaxPreservedPackageBytes =
512 * 1024 * 1024;
...
var exactStyleState =
NeraOpenXmlStyleStateCodec.Read(workbookPart);
...
var sheets =
openXmlWorkbook.AppendChild(new Sheets());
var styleTable =
OpenXmlStyleTable.CreateForExport(workbook);
...
var sheetData =
openXmlWorksheet.GetFirstChild<SheetData>();
...
var formula = string.IsNullOrWhiteSpace(
formulaText)
? null
: $"={formulaText}";
...
? catalog.Intern(
styleTable.GetStyle(styleIndex))
...
foreach (var columns in
openXmlWorksheet.Elements<Columns>())
{
...
var first =
Math.Max(1, (int)minimum.Value) - 1;
var last = Math.Min(
(int)maximum.Value,
...
{
Min = checked(
(uint)(span.StartIndex + 1)),
Max = checked(
(uint)(span.EndIndex + 1)),
...
{
column.Width =
PixelsToExcelColumnWidth(pair.Value);
}
...
private static string ToA1Range(
CellRange range) =>
$"{range.TopLeft.ToA1()}:{range.BottomRight.ToA1()}";
private static double ExcelColumnWidthToPixels(
double width) =>
Math.Max(0d, (width * 7d) + 5d);
private static double PixelsToExcelColumnWidth(
double pixels) =>
