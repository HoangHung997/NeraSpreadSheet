...
internal static class OpenXmlDataValidationPackagePatcher
{
                   ...
                   false))
        {
                ...
                generatedParts.Length != expectedWorksheetCount)
            {
                throw new InvalidOperationException(
                    "Cannot patch data validation when worksheet topology differs.");
            }
    ...
    private static WorksheetPart[] GetWorksheetParts(WorkbookPart workbookPart)
    {
        ...
        WorksheetPart generatedPart)
    {
        ...
        preservedRoot.Elements(
            SpreadsheetNamespace + "dataValidations").Remove();
        foreach (var generated in generatedRoot.Elements(
                     SpreadsheetNamespace + "dataValidations"))
        {
            InsertInSchemaOrder(
                preservedRoot,
                new XElement(generated),
                WorksheetOrder);
        }
