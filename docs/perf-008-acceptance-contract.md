...
| Table filter-button toggle/Undo | Table 10 rows/2 columns, unrelated occupancy 0 hoặc 100.000 cells; 4096/128 (v3) |
| Table structured completion | Cùng fixture, `=Sales[Am`; 32768/1024 (v3) |
| Filter mở catalog | 100.000 rows/unique values, source cap 10.000, page 100; 2/2 |
| Filter cache hit | Trang đầu đã load; 262144/4096 (v3) |
...
Rule version `perf008-v1` được commit trước lần đo đầu tiên. Revision `perf008-v3`
tăng các batch quá ngắn tới dự kiến khoảng 20 ms trở lên theo raw baseline v1:
toggle 4096/128, completion 32768/1024, cached-page 262144/4096. Coordinator chốt
counts này thay approval v2 chỉ completion/cache 32768/1024; quyết định nhận trước
khi xem candidate v2. V2 `97f20261` vừa push khi approval mới đến, không dùng
samples của nó để lựa counts hoặc ghép với v3. Thresholds bên dưới giữ nguyên.
V1/raw/run `33972169896` vẫn INCONCLUSIVE; mỗi revision có baseline/budget riêng.
V3 được commit trước baseline A/A mới và chạy lại toàn ma trận, không chọn một
workload/sample có lợi. Các tỷ lệ
