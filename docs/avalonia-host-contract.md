...
- Icon provider và print-preview control đã có source; chưa có feature-specific
  acceptance. Build/native smoke cũ không thay chứng minh hai feature này.

## Giới hạn bắt buộc giữ OPEN

Full Ribbon/Bars/QAT/customization; Table/filter popups; split host;
...
icon/print-preview feature acceptance; isolated PackageReference consumer and
complete package feed; performance baselines và toàn bộ A2/A3.
Không claim pixel-perfect WPF/Excel. Sample save staging không phải transaction/
atomic disk recovery; shared recovery hold chưa được giải quyết. Native smoke
chạy API trên cửa sổ thật, KHÔNG giả là physical mouse/keyboard/IME/GPU latency
test. Capture dùng loaded visual tree.

## Kiểm thử và bằng chứng

`NeraSpreadSheet.Avalonia.slnx` là solution riêng cho host/sample/tests cùng các
shared dependencies; bao gồm Bars.Core/Ribbon.Core/Iconography để không build
lẫn Debug/Release. Gate mới kiểm closure, output paths và hashes DLL copy vào
sample; không chỉ nhìn tham số `-c Release`. Không sửa solutions host cũ.

MSTest dùng một application/dispatcher trên UI thread riêng mỗi process bằng
public manual-headless APIs. Thread và completion signals được tạo trước
Start, readiness phát từ pump; AssemblyCleanup chờ shutdown và join thread.
Mỗi test vẫn có window/control/workbook riêng, giữ DoNotParallelize và 14
assertion scenarios cũ. Không dùng StartNew chứa task self-capture race của
Avalonia12.1.2; không suppress teardown. Thêm 3 lifecycle regressions gồm 100
vòng đóng/mở/chỉnh sửa; CI chạy cố định 5 process, tất cả phải pass.
[Phân tích và phạm vi bản sửa CI](worklog/AVALONIA-001_CI_FIX.md).

Workflow chạy build/analyzers, headless regressions, architecture, native
window smoke/capture và pack trên Windows/Linux/macOS. Linux native dùng Xvfb;
không thay bằng headless drawing. Success record có source SHA, frame count,
postconditions, OS và loaded assembly location. PackageReference provenance
chưa được claim chỉ vì pack hoặc Release graph thành công.

Chạy từ repository với SDK của global.json:

```powershell
python scripts/verify-avalonia-release.py --self-test
python scripts/verify-avalonia-release.py
...
Rollback: revert bounded CI-fix commit để phục hồi harness trước; toàn bộ
Avalonia vẫn là các paths riêng. Không có workbook schema migration.
