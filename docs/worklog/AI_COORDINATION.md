...
|---|---|---|---|---|---|---|
| `COORD-001` | `READY_FOR_REVIEW` | Codex | `coordination/ai-work-queue` | `docs/worklog/AI_COORDINATION.md` | Không | File có trên GitHub và đọc được bằng raw/API. |
| `Q003B-MAC` | `ACTIVE_EXISTING` | ChatGPT | `feature/bootstrap-architecture-v0.1` | `src/NeraSpreadSheet.Maui/**Apple**`, `src/NeraSpreadSheet.Maui/**MacCatalyst**`, `tests/NeraSpreadSheet.Maui.MacCatalyst.AnalyticsSmoke/**`, `scripts/run-maui-maccatalyst-smoke.sh` | Không | Apple build và Mac Catalyst runtime smoke xanh đúng HEAD. |
| `RIBBON-001` | `READY` | Codex | `ai/codex/ribbon-bars-customization` | `src/NeraSpreadSheet.Ribbon.Core/**`, `src/NeraSpreadSheet.Bars.Core/**`, test riêng của hai module, contract riêng | `COORD-001` được OWNER chấp nhận | Build/test module, architecture verification và branch CI xanh. |
