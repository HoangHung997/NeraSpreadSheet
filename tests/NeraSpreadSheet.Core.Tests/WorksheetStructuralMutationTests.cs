...
public sealed class WorksheetStructuralMutationTests
{
    ...
    public void InsertRowsMovesCellsDimensionsAndMergedRangesTogether()
    {
        ...
        Assert.AreEqual(
            31d,
            sheet.Dimensions.GetRowHeight(4),
            1e-9);
        Assert.IsTrue(sheet.MergedCells.TryGetContaining(
            new CellAddress(8, 3),
            out var merged));
    ...
    public void DeleteColumnsDropsDeletedCellsShiftsOverridesAndShrinksMerge()
    {
        ...
        Assert.AreEqual(
            123d,
            sheet.Dimensions.GetColumnWidth(4),
            1e-9);
        Assert.IsTrue(sheet.MergedCells.TryGetContaining(
            new CellAddress(4, 1),
            out var merged));
    ...
    public void StructuralSnapshotRestoresCellsDimensionsAndMerges()
    {
        ...
        Assert.AreEqual(
            44d,
            sheet.Dimensions.GetRowHeight(3),
            1e-9);
        Assert.IsTrue(sheet.MergedCells.TryGetContaining(
            new CellAddress(5, 1),
            out var merged));
