...
public sealed class SpreadsheetDisplayListComposerTests
{
    ...
    {
        var workbook = new Workbook(); var sheet = workbook.Worksheets[0]; sheet.SetValue(new CellAddress(0, 0), "Nera");
        var rows = new SparseAxisMetricIndex(100, 20d); var columns = new SparseAxisMetricIndex(20, 80d);
        var layout = new ViewportLayoutEngine(rows, columns).Compute(new ViewportRequest(0d, 0d, new SizeD(320d, 200d), 0d));
        var selection = new SelectionModel(); selection.SetActiveCell(new CellAddress(0, 0));
        var displayList = SpreadsheetDisplayListComposer.Compose(WorksheetSnapshot.Capture(sheet), layout, selection.Capture());
