...
public sealed class WorksheetViewStateWindowBindingTests
{
    ...
    public async Task TwoRoundTripsShouldKeepChosenWindowAndIndependentExactOffsets()
    {
        ...
        await serializer.SaveSessionAsync(session, once, new OpenXmlExportOptions { PreserveUnknownParts = true });
        once.Position = 0;
        var firstLoad = await serializer.LoadSessionAsync(once, new OpenXmlImportOptions { PreserveUnknownParts = true });
        await using var twice = new MemoryStream();
        await serializer.SaveSessionAsync(firstLoad, twice, new OpenXmlExportOptions { PreserveUnknownParts = true });
        twice.Position = 0;

        var secondLoad = await serializer.LoadSessionAsync(twice, new OpenXmlImportOptions { PreserveUnknownParts = true });
