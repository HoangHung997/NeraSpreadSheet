...
public sealed class WorksheetViewStatePersistenceTests
{
    ...
    public async Task ExistingUnsplitViewsShouldRetainSiblingIdsUnknownAttributesAndChildren()
    {
        ...
        await serializer.SaveSessionAsync(session, destination, new OpenXmlExportOptions());
        ...
        destination.Position = 0;
        var loaded = await serializer.LoadSessionAsync(destination, new OpenXmlImportOptions());
