...
public sealed class RibbonPresenterTests
{
    ...
    [TestMethod]
    public Task ChoiceShouldKeepSelectedValueSeparateFromHostContext() => AvaloniaTestEnvironment.OnUiAsync(() =>
    {
        using var fixture = new RibbonFixture();
        Assert.IsTrue(fixture.Ribbon.ActivateChoiceAsync("Test.Combo", "a").GetAwaiter().GetResult());
    ...
    [TestMethod]
    public Task DisabledOrUnknownChoiceShouldNotExecute() => AvaloniaTestEnvironment.OnUiAsync(() =>
    {
        using var fixture = new RibbonFixture();
        Assert.IsFalse(fixture.Ribbon.ActivateChoiceAsync("Test.Combo", "disabled").GetAwaiter().GetResult());
        Assert.IsFalse(fixture.Ribbon.ActivateChoiceAsync("Test.Combo", "not-a-choice").GetAwaiter().GetResult());
        Assert.AreEqual(0, fixture.Handler.Count);
    });
    ...
    {
        using var fixture = new RibbonFixture();
        fixture.Ribbon.OpenBackstage();
        fixture.Window.UpdateLayout();
        Assert.IsTrue(fixture.Ribbon.IsBackstageOpen);
        Assert.AreEqual(0, fixture.Handler.Count);
        ((Button)fixture.Find("ribbon-backstage-execute-Test.Button")).RaiseEvent(new RoutedEventArgs(Button.ClickEvent));
        Assert.AreEqual(1, fixture.Handler.Count);
        Assert.IsFalse(fixture.Ribbon.IsBackstageOpen);
    });
    ...
    public Task MinimizedStateShouldUseSharedRuntimeAndSurviveRebuild() => AvaloniaTestEnvironment.OnUiAsync(() =>
    {
        ...
        fixture.Ribbon.Rebuild();
        Assert.IsTrue(fixture.Runtime.IsMinimized);
        Assert.IsNull(((TabItem)fixture.Ribbon.NativeTabControl.SelectedItem!).Content);
    ...
    {
        using var fixture = new RibbonFixture();
        fixture.Ribbon.Width = 100;
        fixture.Window.UpdateLayout();
        fixture.Ribbon.Rebuild();
        fixture.Window.UpdateLayout();
        Assert.IsTrue(fixture.Ribbon.LayoutSnapshot.Tabs[0].HasOverflow);
        Assert.IsNotNull(fixture.Find("ribbon-overflow"));
    });
    ...
    {
        using var fixture = new RibbonFixture();
        using var bar = new NeraBarPresenter(new BarRuntimeController(
            new BarDefinition("test-bar", BarKind.Toolbar, [BarItemDefinition.Command("Test.Button")]), fixture.Registry));
        using var ribbonKeys = fixture.Ribbon.BindShortcuts(fixture.Window);
        using var barKeys = bar.BindShortcuts(fixture.Window);
        fixture.Window.RaiseEvent(new KeyEventArgs
        {
    ...
    {
        using var fixture = new RibbonFixture();
        fixture.Ribbon.EnterKeyTipMode();
        Assert.AreEqual(RibbonKeyTipScope.Tabs, fixture.Ribbon.KeyTipScope);
        fixture.Ribbon.EscapeKeyTipMode();
        Assert.AreEqual(RibbonKeyTipScope.Inactive, fixture.Ribbon.KeyTipScope);
        Assert.AreEqual(0, fixture.Handler.Count);
    });
    ...
    {
        using var fixture = new RibbonFixture();
        var tab = fixture.Ribbon.NativeTabControl.SelectedItem;
    ...
    private sealed class RibbonFixture : IDisposable
    {
        public RibbonFixture()
        {
            ...
            Add("Button", RibbonItemKind.Button);
            Add("Toggle", RibbonItemKind.Toggle);
            Add("Combo", RibbonItemKind.ComboBox);
            Add("Color", RibbonItemKind.ColorPicker);
            Add("Split", RibbonItemKind.SplitButton);
            Add("Menu", RibbonItemKind.Menu);
            Add("Gallery", RibbonItemKind.Gallery);
            ...
            {
                var id = "Test." + name;
                Registry.Register(new CommandDescriptor(id, name, shortcut: name == "Button" ? "Ctrl+S" : null), Handler);
        ...
        public Control Find(string id) => Ribbon.GetVisualDescendants().OfType<Control>()
    ...
    private sealed class TestHandler : IStatefulCommandHandler
    {
        ...
        {
            Count++; LastContext = context; return ValueTask.CompletedTask;
        }
    }
}
